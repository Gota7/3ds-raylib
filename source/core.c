...
typedef struct CoreData {
    struct {
        ...
        Matrix screenScaleBottom;           // Matrix to scale screen (framebuffer rendering)
...
#if defined(PLATFORM_ANDROID) || defined(PLATFORM_RPI) || defined(PLATFORM_DRM) || defined(PLATFORM_UWP) || defined(PLATFORM_3DS)
...
void InitWindow(int width, int height, const char *title)
{
    ...
    if (rc)
    {
        TRACELOG(LOG_WARNING, "Failed to initialize romfs! %08lX", rc);
    }
    CORE.Window.screen.width = CORE.Window.currentFbo.width = width;
    CORE.Window.screen.height = CORE.Window.currentFbo.height = height;
    CORE.Window.display.width = 400;
    CORE.Window.display.height = 240;
    CORE.Window.screenScale = MatrixScale((float)400/CORE.Window.screen.width, (float)240/CORE.Window.screen.height, 1.0f);
    CORE.Window.screenScaleBottom = MatrixScale((float)320/(CORE.Window.screen.width * 0.8), (float)240/CORE.Window.screen.height, 1.0f);
    SetMouseScale((float)CORE.Window.screen.width/400, (float)CORE.Window.screen.height/240);
...
#endif
#if defined(PLATFORM_DESKTOP) || defined(PLATFORM_WEB) || defined(PLATFORM_RPI) || defined(PLATFORM_UWP) || defined(PLATFORM_DRM) || defined(PLATFORM_3DS)
    // Init graphics device (display device and OpenGL context)
    // NOTE: returns true if window and graphic device has been initialized successfully
    // TODO!!! CORE.Window.ready = InitGraphicsDevice(width, height);

    // TODO!!! if (!CORE.Window.ready) return;
...
void BeginDrawing(void)
{
    ...
    rlSetDepth(.99);
    rlSetCurrentScreen(SCREEN_3DS_TOP);
#endif
...
void BeginDrawingBottom(void)
{
    rlSetDepth(.99);
    rlSetCurrentScreen(SCREEN_3DS_BOTTOM);
    rlLoadIdentity();                   // Reset current matrix (modelview)
    rlMultMatrixf(MatrixToFloat(CORE.Window.screenScaleBottom)); // Apply screen scaling
}
...
#if defined(PLATFORM_ANDROID) || defined(PLATFORM_RPI) || defined(PLATFORM_DRM) || defined(PLATFORM_3DS)
    struct timespec ts;
...
static void SetupViewport(int width, int height)
{
    ...
    // NOTE: Configured top-left corner as (0, 0)
#if defined(PLATFORM_3DS)
    rlOrthoTilt(0, CORE.Window.render.width, CORE.Window.render.height, 0, 0.0f, 1.0f);
    //rlRotatef(-90, 0, 0, 1);
    rlMatrixMode(RL_PROJECTION_BOTTOM);
    rlLoadIdentity();
    rlOrtho(0, CORE.Window.render.width * .8, CORE.Window.render.height, 0, 0.0f, 1.0f);
#else
...
// NOTE: GLFW3 joystick functionality not available in web
    ...
    // Previous states.
    for (int i = 0; i < MAX_GAMEPAD_BUTTONS; i++) CORE.Input.Gamepad.previousState[0][i] = CORE.Input.Gamepad.currentState[0][i];

    // Set buttons.
    hidScanInput(); // Scan for 3ds input.
    u32 kDown = hidKeysHeld(); // Get currently pressed buttons.
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_RIGHT_FACE_RIGHT] = (kDown & KEY_A) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_RIGHT_FACE_DOWN] = (kDown & KEY_B) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_MIDDLE_RIGHT] = (kDown & KEY_START) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_MIDDLE_LEFT] = (kDown & KEY_SELECT) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_LEFT_FACE_RIGHT] = (kDown & KEY_DRIGHT) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_LEFT_FACE_LEFT] = (kDown & KEY_DLEFT) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_LEFT_FACE_UP] = (kDown & KEY_DUP) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_LEFT_FACE_DOWN] = (kDown & KEY_DOWN) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_RIGHT_TRIGGER_1] = (kDown & KEY_R) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_LEFT_TRIGGER_1] = (kDown & KEY_L) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_RIGHT_FACE_UP] = (kDown & KEY_X) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_RIGHT_FACE_LEFT] = (kDown & KEY_Y) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_LEFT_TRIGGER_2] = (kDown & KEY_ZL) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_RIGHT_TRIGGER_2] = (kDown & KEY_ZR) > 0;
    CORE.Input.Gamepad.currentState[0][GAMEPAD_BUTTON_UNKNOWN] = (kDown & KEY_TOUCH) > 0;
    CORE.Input.Gamepad.axisState[0][GAMEPAD_AXIS_LEFT_TRIGGER] = (kDown & KEY_L) > 0;
    CORE.Input.Gamepad.axisState[0][GAMEPAD_AXIS_RIGHT_TRIGGER] = (kDown & KEY_R) > 0;

    // Circle pads.
    circlePosition leftCircle;
    hidCircleRead(&leftCircle);
    CORE.Input.Gamepad.axisState[0][GAMEPAD_AXIS_LEFT_X] = (float)leftCircle.dx / 154;
    CORE.Input.Gamepad.axisState[0][GAMEPAD_AXIS_LEFT_Y] = (float)leftCircle.dy / 154;
    circlePosition rightCircle;
    hidCstickRead(&rightCircle);
    CORE.Input.Gamepad.axisState[0][GAMEPAD_AXIS_RIGHT_X] = (float)rightCircle.dx / 156;
    CORE.Input.Gamepad.axisState[0][GAMEPAD_AXIS_RIGHT_Y] = (float)rightCircle.dy / 156;

    // Mouse, or bottom screen touch.
    touchPosition touch;
    hidTouchRead(&touch);
    CORE.Input.Mouse.position.x = (float)touch.px;
    CORE.Input.Mouse.position.y = (float)touch.py;
    CORE.Input.Mouse.cursorOnScreen = (kDown & KEY_TOUCH) > 0;
    CORE.Input.Mouse.previousButtonState[MOUSE_BUTTON_LEFT] = CORE.Input.Mouse.currentButtonState[MOUSE_BUTTON_LEFT];
    CORE.Input.Mouse.currentButtonState[MOUSE_BUTTON_LEFT] = (kDown & KEY_TOUCH) > 0;
...
{
    rlSetCurrentScreen(screen);
}

int GetCurrentScreen(void)
{
    return rlGetCurrentScreen();
}

int GetBottomScreenWidth(void)
{
    return (int)(CORE.Window.screen.width * .8);
}

int GetBottomScreenHeight(void)
{
